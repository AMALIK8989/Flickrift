...
`;document.head.appendChild(g);document.addEventListener("DOMContentLoaded",()=>{const o=["./Assets/js/remove.js","../Assets/js/remove.js"];const t=(o,e=0)=>{if(e>=o.length)return console.warn("⚠️ remove.js not found");const s=document.createElement("script");s.src=o[e];s.defer=true;s.onload=()=>console.log(`✅ remove.js loaded from: ${o[e]}`);s.onerror=()=>{console.warn(`❌ Failed to load remove.js from: ${o[e]}, trying next path...`);t(o,e+1)};document.head.appendChild(s)};t(o)});window.addEventListener("DOMContentLoaded",()=>{const e=["bootstrap.min.js","wow.min.js","font-awesome","jquery-3.6.0.min.js","owl.carousel.min.js","swiper-bundle.min.js"];const n=Array.from(document.querySelectorAll("script[src]"));n.forEach(n=>{const o=n.src;const s=e.some(n=>o.includes(n));if(s){const t=document.createElement("script");t.src=o;t.defer=true;t["async"]=false;document.body.appendChild(t);n.remove()}})});const usercentricsScript=document.createElement("script");usercentricsScript.id="usercentrics-cmp";usercentricsScript.src="https://app.usercentrics.eu/browser-ui/latest/loader.js";usercentricsScript.setAttribute("data-settings-id","CpXNJ6Rw_IrguB");usercentricsScript.async=true;document.head.appendChild(usercentricsScript);document.addEventListener("DOMContentLoaded",()=>{const e="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-9522598653194866";document.querySelectorAll("script[src]").forEach(o=>{if(o.src===e){console.log("Removing AdSense script:",o.src);o.remove()}})});$(function(){console.log("🚀 custom.js booted — searching for navbar.js...");const o=["./Assets/js/navbar.js","../Assets/js/navbar.js","../../Assets/js/navbar.js","../../../Assets/js/navbar.js"];function n(s=0){if(s>=o.length){console.warn("⚠️ navbar.js not found.");return}$.getScript(o[s]).done(()=>console.log(`✅ navbar.js loaded from: ${o[s]}`)).fail(()=>{console.warn(`❌ Failed to load navbar.js from: ${o[s]}`);n(s+1)})}n()});let swiperAssetsLoaded=false;let swiperInitialized=false;function loadSwiperAssets(){if(swiperAssetsLoaded){console.log("ℹ️ Swiper assets already loaded, skipping...");return Promise.resolve()}console.log("🔹 Starting to load Swiper assets...");const cssUrl="https://cdn.jsdelivr.net/npm/swiper@12/swiper-bundle.min.css";const jsUrl="https://cdn.jsdelivr.net/npm/swiper@12/swiper-bundle.min.js";const cssPromise=new Promise((resolve,reject)=>{if(document.querySelector(`link[href="${cssUrl}"]`)){console.log("ℹ️ Swiper CSS already loaded");return resolve()}const link=document.createElement("link");link.rel="stylesheet";link.href=cssUrl;link.onload=()=>{console.log("✅ Swiper CSS loaded");resolve()};link.onerror=err=>{console.error("❌ Failed to load Swiper CSS",err);reject(err)};document.head.appendChild(link)});const jsPromise=new Promise((resolve,reject)=>{if(typeof Swiper!=="undefined"){console.log("ℹ️ Swiper JS already loaded");return resolve()}const script=document.createElement("script");script.src=jsUrl;script.onload=()=>{console.log("✅ Swiper JS loaded");resolve()};script.onerror=err=>{console.error("❌ Failed to load Swiper JS",err);reject(err)};document.body.appendChild(script)});return Promise.all([cssPromise,jsPromise]).then(()=>{swiperAssetsLoaded=true;console.log("✅ All Swiper assets loaded successfully")}).catch(err=>{console.error("❌ Error loading Swiper assets:",err)})}async function initMovieSwiper(){if(swiperInitialized){console.log("ℹ️ Swiper already initialized, skipping...");return}await loadSwiperAssets();const container=document.querySelector(".movieSwiper");if(!container){console.warn("⚠️ Swiper container .movieSwiper not found");return}swiperInitialized=true;new Swiper(container,{slidesPerView:"auto",spaceBetween:10,loop:true,autoHeight:true,autoplay:{delay:3e3,disableOnInteraction:false},pagination:{el:".swiper-pagination",clickable:true},navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"},breakpoints:{300:{slidesPerView:2,spaceBetween:10},375:{slidesPerView:2,spaceBetween:12},400:{slidesPerView:1.2,spaceBetween:14},480:{slidesPerView:1.5,spaceBetween:16},560:{slidesPerView:1.9,spaceBetween:20},640:{slidesPerView:2,spaceBetween:20},768:{slidesPerView:3,spaceBetween:30},1024:{slidesPerView:4,spaceBetween:30},1400:{slidesPerView:5,spaceBetween:30}},observer:true,observeParents:true});console.log("✅ Swiper initialized on .movieSwiper")}document.addEventListener("DOMContentLoaded",()=>{initMovieSwiper();const ref=encodeURIComponent(window.location.href);const links=document.querySelectorAll("a#movie-link");links.forEach(link=>{try{const href=link.getAttribute("href");if(!href)return;const sep=href.includes("?")?"&":"?";link.setAttribute("href",`${href}${sep}ref=${ref}`)}catch(err){console.error("⚠️ Error updating link:",link,err)}});console.log(`✅ Added ?ref=${window.location.href} to ${links.length} movie link(s).`)});document.addEventListener("DOMContentLoaded",async()=>{const e=[{title:"Atomic Blonde",description:"MI6 agent Lorraine Broughton is sent to Cold-War Berlin to recover a priceless dossier amid betrayal and danger.",heroImage:"./Assets/Img/Atomic Blonde.webp",thumbImage:"./Assets/Img/Atomic Blonde Info Image.webp",watchLink:"./Atomic-Blonde.html",trailerLink:"https://www.youtube.com/watch?v=JIqcMl0hzXs"},{title:"Arrow",description:"Oliver Queen returns from a deserted island to become the vigilante Green Arrow in Star City.",heroImage:"./Assets/Img/Arrow Cover.webp",thumbImage:"./Assets/Img/Arrow Info-card.webp",watchLink:"./Tv-shows/Arrow.html",trailerLink:"https://www.youtube.com/watch?v=_a3dNB2riKE"},{title:"Blue Beetle",description:"Jaime Reyes gains alien powers from a mysterious scarab, transforming into Blue Beetle.",heroImage:"./Assets/Img/Blue Beetle .webp",thumbImage:"./Assets/Img/Blue Beetle info-card.webp",watchLink:"./Action/Blue-Beetle.html",trailerLink:"https://www.youtube.com/watch?v=MaCllutk0_w"},{title:"Mission Impossible 1",description:"Ethan Hunt is framed for treason and must uncover the real mole within IMF.",heroImage:"./Assets/Img2/Mission-Impossible-1-Cover.webp",thumbImage:"./Assets/Img/Mission Impossible 1.webp",watchLink:"./Mission-Impossible-1.html",trailerLink:"https://www.youtube.com/watch?v=L8Pbjh4EZRk"},{title:"Oppenheimer",description:"The story of J. Robert Oppenheimer, the father of the atomic bomb.",heroImage:"./Assets/Img10/Oppenheimer-Cover.webp",thumbImage:"./Assets/Img10/Oppenheimer-Info-Card.webp",watchLink:"./Drama/Oppenheimer.html",trailerLink:"https://www.youtube.com/watch?v=uYPbbksJxIg"}];await loadSwiperAssets();const t=document.querySelector(".main-swiper-wrapper");const i=document.querySelector(".thumb-swiper-wrapper");t.innerHTML="";i.innerHTML="";e.forEach(e=>{t.innerHTML+=`
      <div class="swiper-slide">
        <img src="${e.heroImage}" alt="${e.title}" class="carousel-slider-image  W-100" id="hero-image">
        <div class="hero-overlay" id="overlay-wrapper">
          <div class="overlay-content" id="overlay-content">
            <h2 id="overlay-head">${e.title}</h2>
            <p id="overlay-descript">${e.description}</p>
            <div class="cta-group">
              <a href="${e.watchLink}" class="btn btn-in"><i class="fas fa-play"></i> Watch Now</a>
              <a href="${e.trailerLink}" class="btn btn-out"><i class="fas fa-film"></i> Trailer</a>
            </div>
          </div>
        </div>
      </div>
